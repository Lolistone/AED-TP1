...
\begin{proc}{hayFraude}{\In escrutinio\_presidencial : \TLista{\ent}, \In escrutinio\_senadores : \TLista{\ent}, \In escrutinio\_diputados : \TLista{\ent}}{Bool}
	...
	\asegura{res = True \longleftrightarrow \neg (totalVotos(escrutinio\_presidencial) =  totalVotos(escrutinio\_senadores) \land  \\  totalVotos(escrutinio\_senadores) =  totalVotos(escrutinio\_diputados))}
\end{proc}
...
\pred{superaUmbral}{\In escrutinio : \TLista{\ent}, \In i : \ent}{porcentaje(i, escrutinio) > 3}

\pred{dHontValida}{\In DHont : \TLista{\TLista{\ent}}, \In cant\_bancas : \ent, \In escrutinio : \TLista{\ent}}{
cocientesPositivos(DHont) \land cocientesDistintos(DHont)  \land seCorresponde(DHont, cant\_bancas, escrutinio) \\ \land escrutinioValido(escrutinio)}

\pred{seCorresponde}{\In DHont : \TLista{\TLista{\ent}}, \In cant\_bancas : \ent, \In escrutinio : \TLista{\ent}}{
	|escrutinio| - 1 = |DHont| \yLuego \\
	\paraTodo[unalinea]{i}{\ent}{0 \leq i < |DHont| \yLuego superaUmbral(escrutinio, i) \implicaLuego |DHont[i]| =  cant\_bancas} \land \\
	\paraTodo[unalinea]{i}{\ent}{0 \leq i < |Dhont| \yLuego \neg(superaUmbral(escrutinio, i)) \implicaLuego |DHont[i]| =  0} \land \\
	\paraTodo[unalinea]{i,j}{\ent}{0 \leq i < |DHont| \yLuego 0 \leq j < |DHont[i]| \implicaLuego DHont[i][j] =  escrutinio[i]/(j + 1)}
}
...
lista[i][j]_{0} \neq lista[k][r]_{0} } \land \\ \paraTodo[unalinea]{i,j}{\ent}{0 \leq i < |lista| \yLuego 0 \leq j < |lista[i]| \implicaLuego lista[i][j]_{0} > 0}}
