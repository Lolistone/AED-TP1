...
\begin{proc}{hayBallotage}{\In escrutinio : \TLista{\ent}}{Bool}
	%    \modifica{parametro1, parametro2,..}
	\requiere{|escrutinio| > 2 \yLuego totalVotos(escrutinio) > 0 \yLuego noHayEmpate(escrutinio)}
	\asegura{res = True \longleftrightarrow \neg (\existe[unalinea]{!i}{\ent}{0 \leq i < |escrutinio| - 1 \yLuego porcentaje(i, escrutinio)  > 45} \lor \\
		     (\paraTodo[unalinea]{j,k}{\ent}{0 \leq j,k < |escrutinio| - 1  \yLuego porcentaje(j, escrutinio) > 40  \implicaLuego \\
		     porcentaje(j, escrutinio) - porcentaje(k, escrutinio) > 10}))}
\end{proc}
\vspace{0.3cm}
...
\begin{proc}{nombre}{\In escrutinio\_presidencial : \TLista{\ent}, \In escrutinio\_senadores : \TLista{\ent}, \In escrutinio\_diputados : \TLista{\ent}}{Bool}
	%    \modifica{parametro1, parametro2,..}
	\requiere{|escrutinio\_presidenciales| > 2 \land |escrutinio\_senadores| > 2 \land |escrutinio\_diputados| > 2 \yLuego \\ totalVotos(escrutinio\_presidencial) > 0 \land totalVotos(escrutinio\_senadores) > 0 \land \\ totalVotos(escrutinio\_diputados) > 0 }
