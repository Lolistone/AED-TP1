...
Comencemos eligiendo, \vspace{0.3cm} \\
$\textbf P_{c} \equiv escrutinioValido(escrutinio) \land i = 1 \land 0 \leq primero, segundo < 2 \land  primero \neq segundo \land \\ escrutinio[segundo] <escrutinio[primero] $ \vspace{0.1cm} \\
...
(Es la guarda del ciclo) \vspace{0.1cm} \\
$\textbf I \equiv 0 < i \leq |escrutinio| - 1 \yLuego  0 \leq primero, segundo < i + 1 \yLuego \paraTodo[unalinea]{j}{\ent}{0 \leq j < i \land j \neq primero \land 
...
\noindent $\equiv$ def(i := 1) $\yLuego$  escrutinioValido(escrutinio) $\land$ 1 = 1 $\land$ 0 $\leq$ primero, segundo $<$ 2 $\land  primero \neq segundo$ $\land$ \\ escrutinio[segundo] $<$ escrutinio[primero] \\
...
(escrutinio[0] $<$ escrutinio[1] $\land$ wp(primero := 1; segundo := 0; $E_1$)))  \vspace{0.1cm} \\
...
\begin{itemize}\setlength{\itemindent}{0.5cm}
	...
	\item $0 \leq primero, segundo < i + 1 $ vale pues i = 1 y la expresion aparece en $P_C$
	\item  $\paraTodo[unalinea]{j}{\ent}{0 \leq j < i \land j \neq primero \land  j \neq segundo \implicaLuego  escrutinio[j] < escrutinio[segundo]}  (1)$
	\item escrutinio[segundo] $\leq$ escrutinio[primero] vale pues aparece en la pre del ciclo.
	\item noHayEmpate(escrutinio) vale pues esta dento de escrutinioValido(escrutinio)
\end{itemize}

\noindent (1) Como i = 1, el único valor que cumple 0 $\leq j < i$ es j = 0, pero como primero y segundo son 1 y 0 respectivamente, o viceversa pues estan entre 0 y 1 inclusive y primero es distinto de segundo todo el antecedente es falso.
Por otro lado, para todo j $\neq$ 0 el antecedente también es falso. Luego, la implicación siempre es verdadera. \\
...
\noindent (escrutinio[i] $>$ escrutinio[segundo] y escrutinio[i] $\leq$ escrutinio[primero] valen por la hipótesis, y noHayEmpate(escrutinio) es trivial) \\
...
\noindent (Los indices estan en rango) \\

\noindent $E_8 \equiv
(escrutinio[i] \leq escrutinio[segundo] \land \\
\paraTodo[unalinea]{j}{\ent}{0 \leq j < i + 1 \land j \neq primero \land j \neq segundo \implicaLuego escrutinio[j] < escrutinio[segundo]} \\ \land escrutinio[segundo] \leq escrutinio[primero] \land noHayEmpate(escrutinio))$ \\

\noindent (escrutinio[i] $\leq$ escrutinio[segundo] y escrutinio[segundo] $\leq$ escrutinio[primero] salen de la hipótesis) \\
